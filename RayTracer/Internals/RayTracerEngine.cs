...
namespace RayTracer.Internals {
    ...
    public class RayTracerEngine {
        ...
        private Color Shade(ISect isect, Scene scene, int depth) {
            ...
            Vector pos = Vector.Times(isect.Dist, isect.Ray.Dir) + isect.Ray.Start;
            Vector normal = isect.Thing.Normal(pos);
            Vector reflectDir = d - Vector.Times(2 * Vector.Dot(normal, d), normal);
            ...
            return Color.Plus(ret, GetReflectionColor(isect.Thing, pos + Vector.Times(.001, reflectDir), normal, reflectDir, scene, depth));
        }
        ...
        private Vector GetPoint(double x, double y, Camera camera) {
            return Vector.Norm(camera.Forward + Vector.Times(RecenterX(x), camera.Right) + Vector.Times(RecenterY(y), camera.Up));
        }
        ...
        public SDBitmap Render(Scene scene) {
            ...
            if (OnUpdateStatus != null)
                ...
                stop.TotalMilliseconds.ToString("0")));
            return bitmap;
        }
    }
}
